...
### ⚠️ 媒體斷點有 12 種，**那不是債，不要去收它**

```
768 ×18(201 條宣告)   900 ×7(164)   640 ×11(66)   720 ×5(56)
480 ×8(42)            1024 ×5(21)   950 ×2(12)    600 ×3(9)
1100 ×2(5)            860 ×1(5)     560 ×1(3)     1300 ×1(1)
```

看起來跟間距那 87 種值是同一種病，**但不是**。間距的問題是「同一個角色沒有標準
答案」；斷點的值是**由那個元件自己的內容寬度決定的**——「這個 grid 在這裡塞不下了」。
逐一查過六個只用 1–3 次的：

| 斷點 | 在做什麼 |
|---|---|
| 1300 / 1100 / 950 | `article-shell` 三欄佈局的**三段崩解階梯**（先收左側欄，再收…） |
| 860 | `HomeLately` 的 `.lately-grid` 收成單欄 |
| 600 | `InfoPage` 的朋友清單收成單欄 |
| 560 | `FavoritesEditor` 的 `.fe-add-row` 換行 |

把左側欄從 1300 改成 1024 才消失，是為了數字好看而改變版面崩解的位置。

另外掃過「同一個選擇器出現在多個斷點」的 16 個案例（`.blog-content-wrapper`
1024→768→480、`.exif-panel` 1024→768→640…），**全部是同一個元件的漸進崩解**，
不是不一致。也沒有任何「同一角色、不同檔案、隨手挑不同斷點」的案例。

⚠️ 而且 `computed-style` **蓋不到斷點改動**：它只跑 1280 / 768 / 390 三個寬度，
把某條規則從 900 改到 1024，變的是 901–1024 那一段，三個寬度一個都沒踩到。
真要動斷點就得另外寫「掃寬度」的驗證。
