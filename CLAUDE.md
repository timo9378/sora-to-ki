...
不是許可**，而且 2026-09-12 真的照這句話清過一次：29 條 → 19 條、137 次 → 53 次。
...
2. **「版面尺寸硬貼會走樣」** —— 只對 `120px`（最近的尺 96px，差 24px）成立。
   `26/30/42/50/55px` 離最近一格只有 1–2px，那是債不是原則。

現在名單裡剩下的 19 條只有兩類，各有真的理由：**負值**（沒有負的 token 形式）與
**頁面外框的上下留白**（見下面）。

⚠️ **`60/70/72/88/90/100/120px` + `5.5rem` / `8rem` 是同一個待辦，不是九個零散的豁免。**
那 24 條宣告全是同一個角色——固定頂欄底下的頁面上下留白，出現在 `.info-page` /
`.tk-page` / `.w-page` / `.wl-page` / `.setup-content` / `.blog-` `.bookshelf-`
`.music-` `.activity-content-wrapper` / `.nf-page` / `.app-footer`——而桌機版有
**90 / 100 / 120 三種值**、窄視窗有 **60 / 70 / 100 三種**。那是「每個頁面自己編一個」，
不是版面必要的差異。要收就是訂一組 `--page-top` / `--page-bottom`（分桌機與窄視窗），
但那會讓 8 個頁面的內容上下各移動最多 30px，**是設計決定不是 codemod 的事**。
