...
間距尺定義在 `src/index.css` 的 `@theme`（`--space-px` 到 `--space-24`，2px 格線到
24px，之後 4px／8px 步進）。`margin` / `padding` / `gap` 不准再寫字面值。

⚠️ **這道不是潔癖。** 補它之前這個專案**沒有間距尺**：87 種不同的長度值、1594 次、
token 覆蓋 0，而且長出 `0.15rem` / `0.35rem` / `0.45rem` / `0.55rem` 這種不屬於任何
體系的即興值——沒有人「決定」要用 5.6px，是打了 `0.35rem` 覺得差不多。同一個尺寸還被
px 與 rem 各寫一遍（15 個尺寸都是）。收斂那 1456 處花了兩支 PR。
