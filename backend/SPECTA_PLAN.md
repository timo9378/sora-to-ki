...
| admin.rs comments | 1 | `admin_comments` 仍是 `row_to_json`。**前端 `CommentsManager` 卡在這裡**（沒有生成型別可用），下一棒最順手 |
...
2. **`admin_comments`**（← 下一棒；解鎖前端 CommentsManager）
3. thoughts / books（中型、模式重複）
4. collection（先確認後端要不要退役）
5. watch favorites / newsletter / oauth user（小型）
6. 全清後：刪 `util::row_to_json`（`js_normalize_numbers` **不能刪**——
   thirdparty/spotify/watch 的透傳路徑仍在用）、
