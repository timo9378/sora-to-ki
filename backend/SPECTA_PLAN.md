...
| ~~newsletter.rs~~ | ~~1~~ | **✅ 2026-07-18 完成**：`SubscriberRow`（7 欄）+ `SubscribersResponse`（reuse `posts::Pagination`）+ `SubscriberByToken`。前端 SubscribersManager `Subscriber` interface 歸零 |
| thoughts.rs | 1 | **決定不做（保持現狀）**：`ref` 是 passthrough Value（存使用者原始 JSON、任意 key），typed 化會丟未知 key → 形狀變；符合「透傳不 typed」原則。backend 已有 `ThoughtOut` 等結構，前端 `ThoughtRef` 是前端 view-model（非 API 契約），留手寫合理 |
| spotify/oauth/quote/bahamut | 1–2 | 小額收尾（多為第三方透傳，多半保持 Value） |
...
3. ~~books~~ **✅ 完成**
4. ~~newsletter~~ **✅ 完成**
5. ~~thoughts~~ **決定不做**（ref passthrough，見上表 + 坑清單）
6. **collection**（← 下一棒；先確認後端要不要退役）
7. watch favorites / oauth user（小型）
8. 全清後：刪 `util::row_to_json`（**只剩 collection.rs、watch.rs 在 call**；
