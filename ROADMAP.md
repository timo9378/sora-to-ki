...
**B6. API 文件 utoipa + Scalar — 🚧 首波完成（2026-07-19）**
✅ 公開讀端點（11 個 GET）+ 23 個 response struct `utoipa::ToSchema`（與 specta::Type 並存、型別單一來源）→ `GET /api/openapi.json`（自架 spec）+ `GET /api/docs`（Scalar UI）。
⚠️ **不用 utoipa-scalar crate**（綁 axum 0.8、本專案 axum 0.7 → 自 serve spec JSON + Scalar HTML；UI 的 JS 走 CDN）。
⏳ 待增量：admin / mutation / 第三方端點（~110 個）——照需求逐步加 `#[utoipa::path]`（struct 多半已有 ToSchema 或補一個 derive 即可）。
成本：首波已付；剩餘增量中。價值：中（自己 debug + 未來有用）。
