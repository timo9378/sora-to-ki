...
// icon/color/tags 穩定,title/subtitle/description 隨語系。year 多為數字也穩定,「現在」「未來」隨語系
...
const Journey = () => {
  ...
  const milestonesLocale = MILESTONES_BY_LANG[lang] || MILESTONES_BY_LANG['zh-TW'];
  const milestones = MILESTONE_STATIC.map((s, i) => {
    const tr = milestonesLocale[i];
    const year = s.year === 'NOW' ? ylabels.NOW : s.year === 'FUTURE' ? ylabels.FUTURE : s.year;
    return { ...s, year, title: tr.title, subtitle: tr.subtitle, description: tr.description, tags: [...s.tags, ...(tr.extraTags || [])] };
  });
  ...
  return (
    <div className="journey-page" ref={containerRef}>
      <SEOHead title={({ 'zh-TW':'旅程','zh-CN':'旅程',en:'Journey',ja:'歩み',ko:'여정' })[lang]} description={({ 'zh-TW':'Koimsurai 的成長與學習旅程時間線。','zh-CN':'Koimsurai 的成长与学习旅程时间线。',en:"Koimsurai's growth and learning journey timeline.",ja:'Koimsurai の成長と学びの軌跡。',ko:'Koimsurai의 성장과 학습 여정.' })[lang]} path="/journey" />
      ...
      <div className="journey-content-wrapper">
        ...
        >
          <motion.h1 className="journey-title" variants={fadeUp}>
            <span className="title-gradient">{ui.title}</span>
          </motion.h1>
            ...
            return (
              ...
              >
                ...
                <div className="section-text">
                  ...
                  >
                    {isActive ? ui.collapse : ui.expand}
                  </button>
                </div>
              </motion.section>
            );
        ...
        >
          <h2 className="ending-title">{ui.endingTitle}</h2>
